...
public class InteractionPortal : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        // player가 null인지 여기서 직접 확인!
        if (player == null)
        {
            Debug.LogError("### CRITICAL ERROR: Player를 찾지 못했습니다! 태그를 확인하세요!");
        }
    ...
    private void Update()
    {
        ...
        if (playerInRange && Input.GetKeyDown(interactionKey))
        {
            ...
            Debug.Log("=== 포탈 W키 입력 감지! 모든 입력 즉시 차단 ===");
    ...
    private void OnTriggerEnter2D(Collider2D other)
    {
        if (other.CompareTag("Player"))
        {
            ...
            {
                float remainingCooldown = TeleportCooldownManager.Instance.GetRemainingCooldown();
                Debug.Log($"{portalName} 범위 진입! (공유 쿨타임 {remainingCooldown:F1}초 남음)");
            }
            else
            {
                Debug.Log($"{portalName} 범위 진입! {interactionKey}키를 눌러 {destinationName}로 이동하세요.");
            }
        }
    }
    ...
    private void OnTriggerExit2D(Collider2D other)
    {
        ...
        {
            playerInRange = false;
            Debug.Log($"{portalName} 범위 이탈");
        }
    }
    
    /// <summary>
    /// 텔레포트 시퀀스 (간단하고 확실한 버전)
    /// </summary>
    private IEnumerator TeleportSequence()
    {
        ...
        {
            TeleportCooldownManager.Instance.StartTeleport(); // 공유 쿨타임 즉시 시작
        }
        
        Debug.Log("=== 포탈 텔레포트 시작! 공유 쿨타임 적용 ===");
        ...
        {
            // 위치 이동
            player.transform.position = new Vector3(teleportTarget.position.x, teleportTarget.position.y, 0);
            ...
            if (rb != null)
            {
                ...
                Debug.Log("포탈: 이동 후 물리 상태 완전 초기화");
            }
        }
    ...
    private IEnumerator FadeScreen(bool fadeOut)
    {
        ...
        Debug.Log($"포탈 페이드 애니메이션 시작: {startAlpha} → {targetAlpha}, 예상 시간: {fadeTime}초");
        ...
        Debug.Log($"포탈 페이드 완료: 최종 알파 = {targetAlpha}, 실제 소요 시간: {Time.unscaledTime - startTime:F3}초");
        
        Debug.Log($"페이드 완료: 최종 알파 = {targetAlpha}");
    ...
    private void DisablePlayerControl()
    {
        ...
        if (characterMove != null) 
        {
            ...
            Debug.Log("포탈: CharacterMove 상태 초기화");
        }
        
        Debug.Log("=== 포탈: 플레이어 제어 완전 비활성화 완료 ===");
    }
    ...
    private void EnablePlayerControl()
    {
        ...
        Debug.Log("=== 포탈: 플레이어 제어 완전 활성화 시작 ===");
    ...
    private IEnumerator DelayedControlRestore()
    {
        ...
        if (characterMove != null) 
        {
            ...
            Debug.Log("포탈: CharacterMove 상태 완전 초기화 및 스프라이트 오른쪽 설정");
        }
